...
export function createDesktopPlatform(): Platform {
  ...
  return {
    ...
    inAppBrowser: {
      ...
      unmarkCitation: window.api.unmarkInAppBrowserCitation,
      revealCitation: window.api.revealInAppBrowserCitation,
    ...
    async trashNoteFile(path) {
      await window.api.trashNoteFile(path)
    },
    getFileIcon(path) {
