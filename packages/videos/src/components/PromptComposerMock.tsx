...
const SURFACE_SHADOW = "0 1px 2px -1px rgba(0,0,0,0.08), 0 12px 28px -12px rgba(0,0,0,0.18)"
...
const getFramesPerCharacter = ({
  ...
  return (fps * SECONDS_PER_MINUTE) / (wordsPerMinute * APPROXIMATE_CHARACTERS_PER_WORD)
}
...
export const PromptComposerMock = ({
  ...
  const submitAnimationFrame = submitFrame === undefined ? 0 : Math.max(0, frame - submitFrame)
  ...
  const submitPulseOpacity = submitHasStarted
    ? interpolate(submitAnimationFrame, [0, SUBMIT_PULSE_END_FRAME], [0.45, 0], {
        easing: SUBMIT_EASING,
        extrapolateLeft: "clamp",
        extrapolateRight: "clamp",
      })
  ...
  const submitPulseScale = submitHasStarted
    ? interpolate(submitAnimationFrame, [0, SUBMIT_PULSE_END_FRAME], [0.9, SUBMIT_PULSE_SCALE], {
        easing: SUBMIT_EASING,
        extrapolateLeft: "clamp",
        extrapolateRight: "clamp",
      })
  ...
  const submitArrowOpacity = submitHasStarted
    ? interpolate(submitAnimationFrame, [SUBMIT_PRESS_FRAME, SUBMIT_ARROW_LIFT_END_FRAME], [1, 0], {
        easing: SUBMIT_EASING,
        extrapolateLeft: "clamp",
        extrapolateRight: "clamp",
      })
      ...
      >
        ...
        >
          ...
          <div
            style={{
              alignItems: "center",
              backgroundColor: canSubmit ? COLORS.sendEnabled : COLORS.sendDisabled,
              borderRadius: "50%",
              display: "flex",
              height: CONTROL_SIZE,
              justifyContent: "center",
          ...
          >
            ...
            >
              <ArrowUpIcon
                color={canSubmit ? COLORS.sendEnabledIcon : COLORS.sendDisabledIcon}
                size={16}
              />
            </div>
          </div>
