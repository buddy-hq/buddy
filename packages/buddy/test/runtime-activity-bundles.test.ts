...
import { resolveIntentPermissions } from "../src/learning/intents/capabilities"
...
describe("resolveIntentPermissions", () => {
  ...
  test("auto returns deduped union scoped by persona and workspace", () => {
    ...
    expect(Object.keys(permissions.tools).every((toolId) => !toolId.startsWith("activity_"))).toBe(true)
  })
