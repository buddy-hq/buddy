...
function withLearnerMemoryEnabled(testBody: () => Promise<void>): () => Promise<void> {
  return () => runWithLearnerMemoryLabContext({ settingsOverride: { enabled: true } }, testBody)
}

describe("learner context delivery", () => {
  test(
    "emits bootstrap learner context on first delivery",
    withLearnerMemoryEnabled(async () => {
  ...
  test(
    "does not emit learner context when the delivered fingerprint has not changed",
    withLearnerMemoryEnabled(async () => {
      await using project = await tmpdir({ git: true })
      const config = await readProjectConfig(project.path)
  ...
  test(
    "emits a learner context delta after learner memory changes",
    withLearnerMemoryEnabled(async () => {
      await using project = await tmpdir({ git: true })
      const config = await readProjectConfig(project.path)
  ...
  test(
    "preserves delta delivery after an unchanged intermediate turn",
    withLearnerMemoryEnabled(async () => {
      await using project = await tmpdir({ git: true })
      const config = await readProjectConfig(project.path)
  ...
  test(
    "records learner context delivery only after the transformed prompt is accepted",
    withLearnerMemoryEnabled(async () => {
      await using project = await tmpdir({ git: true })
  ...
  test(
    "accepted delivery does not create or strengthen learner memories",
    withLearnerMemoryEnabled(async () => {
      await using project = await tmpdir({ git: true })
