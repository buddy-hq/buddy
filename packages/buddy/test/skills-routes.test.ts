...
`,
  ...
  writeFileSync(path.join(releaseNotesSkillDir, "references", "template.md"), "## Release notes template\n")
...
`,
    ...
    try {
      ...
      expect(afterToggleBody.library.some((entry) => entry.id === "release-notes" && entry.installed === false)).toBe(
        true,
      )
