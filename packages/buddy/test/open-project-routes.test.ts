...
describe("open project routes", () => {
  ...
  test("recovers corrupt desktop-notebooks.json from the backup registry", async () => {
    await using repo = await createGitRepo("buddy-route-open-project-backup-recovery")
    const repoPath = realpathSync(repo.path)

    mkdirSync(path.dirname(registryPath), { recursive: true })
    writeFileSync(registryPath, "{", "utf8")
    writeRegistryFile(registryBackupPath, [repoPath])

    const response = await app.request("/api/open-projects")

    expect(response.status).toBe(200)
    await expect(response.json()).resolves.toEqual({ directories: [repoPath] })
    expect(readRegistryFile()).toEqual([repoPath])
    expect(readRegistryBackupFile()).toEqual([repoPath])
    expect(corruptRegistryFiles()).toHaveLength(1)
  })

  test("restores a missing desktop-notebooks.json from the backup registry", async () => {
    await using repo = await createGitRepo("buddy-route-open-project-missing-recovery")
    const repoPath = realpathSync(repo.path)
    writeRegistryFile(registryBackupPath, [repoPath])
  ...
  test("resolves relative directories against BUDDY_DIRECTORY_BASE when configured", async () => {
    await using repo = await createGitRepo("buddy-route-open-project-base")
    const canonicalRepo = realpathSync(repo.path)
    const base = path.dirname(repo.path)
    const relativeDirectory = path.basename(repo.path)
  ...
  test("allows explicit folder opening outside configured allowed roots", async () => {
    await using repo = await createGitRepo("buddy-route-open-project-reject")
    const canonicalRepo = realpathSync(repo.path)
    const originalAllowedRoots = process.env.BUDDY_ALLOWED_DIRECTORY_ROOTS

    process.env.BUDDY_ALLOWED_DIRECTORY_ROOTS = path.join(repo.path, "different-root")
  ...
  test("allows project-scoped routes for directories present in the open project registry", async () => {
    await using repo = await createGitRepo("buddy-route-open-project-registry-allow")
    const canonicalRepo = realpathSync(repo.path)
    const originalAllowedRoots = process.env.BUDDY_ALLOWED_DIRECTORY_ROOTS
    process.env.BUDDY_ALLOWED_DIRECTORY_ROOTS = path.join(repo.path, "different-root")
  ...
  test("does not allow nested directories unless they are explicitly opened", async () => {
    await using repo = await createGitRepo("buddy-route-open-project-registry-exact")
  ...
  test("supports wildcard allowed roots", async () => {
    await using repo = await createGitRepo("buddy-route-open-project-wildcard")
    const canonicalRepo = realpathSync(repo.path)
    const originalAllowedRoots = process.env.BUDDY_ALLOWED_DIRECTORY_ROOTS
  ...
  test("opening is idempotent and preserves existing order", async () => {
    await using firstRepo = await createGitRepo("buddy-route-open-project-first")
    await using secondRepo = await createGitRepo("buddy-route-open-project-second")
    const firstRepoPath = realpathSync(firstRepo.path)
    const secondRepoPath = realpathSync(secondRepo.path)
    const originalAllowedRoots = process.env.BUDDY_ALLOWED_DIRECTORY_ROOTS

    process.env.BUDDY_ALLOWED_DIRECTORY_ROOTS = [
      path.dirname(firstRepoPath),
      path.dirname(secondRepoPath),
    ].join(",")
    ...
    try {
      await app.request("/api/open-projects", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ directory: firstRepoPath }),
      })
      await app.request("/api/open-projects", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ directory: secondRepoPath }),
      })
      ...
      expect(readRegistryFile()).toEqual([secondRepoPath, firstRepoPath])
    } finally {
      if (originalAllowedRoots === undefined) delete process.env.BUDDY_ALLOWED_DIRECTORY_ROOTS
      else process.env.BUDDY_ALLOWED_DIRECTORY_ROOTS = originalAllowedRoots
    }
  ...
  test("closing is idempotent", async () => {
    await using firstRepo = await createGitRepo("buddy-route-open-project-close-first")
    await using secondRepo = await createGitRepo("buddy-route-open-project-close-second")
    const firstRepoPath = realpathSync(firstRepo.path)
    const secondRepoPath = realpathSync(secondRepo.path)
    const originalAllowedRoots = process.env.BUDDY_ALLOWED_DIRECTORY_ROOTS
    ...
    try {
      ...
      const closeResponse = await app.request(
        `/api/open-projects?directory=${encodeURIComponent(firstRepoPath)}`,
        {
          method: "DELETE",
        },
      )
      expect(closeResponse.status).toBe(200)
      expect(readRegistryFile()).toEqual([secondRepoPath])
  ...
  test("reorders the current registry and rejects set mismatches", async () => {
    await using firstRepo = await createGitRepo("buddy-route-open-project-order-first")
    await using secondRepo = await createGitRepo("buddy-route-open-project-order-second")
    const firstRepoPath = realpathSync(firstRepo.path)
    const secondRepoPath = realpathSync(secondRepo.path)
    const originalAllowedRoots = process.env.BUDDY_ALLOWED_DIRECTORY_ROOTS
    ...
    try {
      ...
      const reorderResponse = await app.request("/api/open-projects/order", {
        method: "PUT",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ directories: [firstRepoPath, secondRepoPath] }),
      })

      expect(reorderResponse.status).toBe(200)
      await expect(reorderResponse.json()).resolves.toEqual({
        directories: [firstRepoPath, secondRepoPath],
      })
      expect(readRegistryFile()).toEqual([firstRepoPath, secondRepoPath])

      const mismatchResponse = await app.request("/api/open-projects/order", {
        method: "PUT",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ directories: [firstRepoPath] }),
      })
  ...
  test("listing open projects does not create a project for the backend cwd", async () => {
    await using repo = await createGitRepo("buddy-route-open-project-readonly")
    const canonicalRepo = realpathSync(repo.path)
  ...
  test("stores normalized directories from the live desktop-notebooks file format", async () => {
    await using repo = await createGitRepo("buddy-route-open-project-normalize")
    const nested = path.join(repo.path, "nested")
    mkdirSync(nested, { recursive: true })
    const canonicalRepo = realpathSync(repo.path)

    const originalAllowedRoots = process.env.BUDDY_ALLOWED_DIRECTORY_ROOTS
    process.env.BUDDY_ALLOWED_DIRECTORY_ROOTS = path.dirname(repo.path)
