...
function runGit(args: string[], cwd?: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      reject(new Error(output.length > 0 ? output : `git ${args.join(" ")} failed with exit code ${code}`))
    })
  })
}
...
function parseCuratedSkillDocument(input: {
  ...
  const summarizedContent = summarizeContent(parsed.content) ?? `Use the ${skillName} skill for this workflow.`
  ...
  const examplePrompt =
    readOptionalString(parsed.data["example_prompt"]) ?? `Use the ${skillName} skill to help with this task.`
...
async function readCuratedSkillsFromRepository(repositoryRoot: string): Promise<CuratedLibrarySkill[]> {
