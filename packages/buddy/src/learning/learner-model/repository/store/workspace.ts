...
export async function patchWorkspaceContext(
  ...
  const next: WorkspaceContextArtifact = {
    ...
    ...(patch.tags !== undefined
      ? { tags: normalizeList(patch.tags.map((tag) => tag.toLowerCase())) }
      : {}),
    ...(patch.pinnedGoalIds !== undefined ? { pinnedGoalIds: [...patch.pinnedGoalIds] } : {}),
    ...(patch.projectConstraints !== undefined
      ? { projectConstraints: normalizeList(patch.projectConstraints) }
      : {}),
    ...(patch.localToolAvailability !== undefined
      ? { localToolAvailability: normalizeList(patch.localToolAvailability) }
      : {}),
    ...(patch.preferredSurfaces !== undefined
      ? { preferredSurfaces: [...patch.preferredSurfaces] }
      : {}),
