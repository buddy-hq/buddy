...
type RuntimePromptProfile = Pick<
  SystemPromptCtx,
  "persona" | "capabilityEnvelope"
>;
...
function buildLearnerContextSections(snapshot: LearnerSnapshotContext): {
  ...
  const progressLines = [
    ...(snapshot.sections.find((section) => section.title === "Next Step")
      ?.items ?? ["No active plan decision is available yet."]),
...
function buildWorkspaceStateText(
  ...
  const hasEditor =
    profile.capabilityEnvelope.visibleSurfaces.includes("editor");
  const hasFigure =
    profile.capabilityEnvelope.visibleSurfaces.includes("figure");
...
function buildCalculatorRuntimeText(
  profile: RuntimePromptProfile,
): string | undefined {
...
function buildResourceContextText(
  resources: SystemPromptCtx["resources"],
): string {
  const lines = [
  ...
  const detailedResources = resources.slice(
    0,
    RESOURCE_INVENTORY_DETAILED_MAX_ITEMS,
  );
...
async function buildRuntimeContext(
  input: SystemPromptCtx,
): Promise<RuntimeContextBuild> {
  const workspaceState: WorkspaceState = input.teachingContext?.active
    ? "interactive"
    : "chat";
