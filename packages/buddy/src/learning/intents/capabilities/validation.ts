...
function assertNoRegistryCollisions(input: ValidationInput) {
  const duplicateSkillKeys = duplicateValues(input.skillCapabilities.map((capability) => capability.key))
  if (duplicateSkillKeys.length > 0) {
    throw new Error(`Duplicate skill capability keys detected: ${duplicateSkillKeys.join(", ")}`)
  }

  const duplicateToolIds = duplicateValues(input.toolCapabilities.map((capability) => capability.tool.id))
  if (duplicateToolIds.length > 0) {
    throw new Error(`Colliding pedagogy tool IDs detected across capabilities: ${duplicateToolIds.join(", ")}`)
  }

  const duplicateSkillNames = duplicateValues(input.skillCapabilities.map((capability) => capability.skillName))
  if (duplicateSkillNames.length > 0) {
    throw new Error(`Colliding skill names detected across capabilities: ${duplicateSkillNames.join(", ")}`)
  }
}

function assertManifestIntegrity(input: ValidationInput) {
  const knownToolCapabilityKeys = new Set(input.toolCapabilities.map((capability) => toolCapabilityKey(capability)))
  const knownSkillCapabilityKeys = new Set(input.skillCapabilities.map((capability) => capability.key))
  const crossTypeTopics = new Map<string, Set<"tool" | "skill">>()

  for (const manifest of input.manifests) {
    const duplicateToolKeys = duplicateValues(manifest.toolCapabilities.map((capability) => toolCapabilityKey(capability)))
    ...
    for (const key of manifest.toolCapabilities.map((capability) => toolCapabilityKey(capability))) {
