...
- `entryPath` is only the entry HTML file inside a folder `path`, such as `index.html`; it is null when `path` already points at an HTML file.
- `description` is optional learner-facing summary text; it is not a substitute for `title`.
- `viewportPreset` is required for `present_path` and null for `present_object`.
