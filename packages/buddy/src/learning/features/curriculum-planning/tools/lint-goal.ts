import LINT_DESCRIPTION from "./lint-goal.md"
import z from "zod"
...
type GoalLintInput = {
  scope: GoalScope
  goals: Goal[]
  explicitlyRequestedSingleGoal: boolean
}

function createGoalLintReport(params: GoalLintInput): GoalLintReport {
  const errors: GoalLintIssue[] = []
  const warnings: GoalLintIssue[] = []
  ...
  for (const [index, goal] of params.goals.entries()) {
    ...
    if (!hasGoalTemplatePrefix(scope, goal.statement)) {
      pushIssue({
        ...
        severity: "error",
        message:
          "Use the canonical form: 'At the end of this course/topic, you will be able to …'.",
      })
    }
    ...
    if (isLikelyCompoundTask(goal.statement) || isLikelyCompoundTask(goal.task)) {
      pushIssue({
        ...
        severity: "error",
        message:
          "Split compound outcomes into separate goals so each goal targets one primary performance.",
      })
    }
    ...
    if (!isLikelyTestableTask(goal.howToTest)) {
      pushIssue({
        ...
        severity: "error",
        message:
          "The mastery check must describe a concrete performance and the conditions for success.",
      })
    }
    ...
    if (isLikelyWeakRelevance(goal.task)) {
      pushIssue({
        ...
        severity: "warning",
        message:
          "The goal would be stronger if the task were tied to a more concrete, useful outcome.",
      })
    }
  }
  ...
  return GoalLintReportSchema.parse({
    ok: errors.length === 0,
    errors,
    warnings,
    summary,
...
const goalLintTool = createBuddyTool({
  ...
  async execute(params, ctx: BuddyToolContext) {
    ...
    const result = createGoalLintReport({
      scope: params.scope,
      goals: params.goals,
      explicitlyRequestedSingleGoal: params.explicitlyRequestedSingleGoal,
    })
...
export { createGoalLintReport, goalLintTool }
