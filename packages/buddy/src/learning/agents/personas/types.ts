export { PERSONAS, PERSONA_SURFACES, isPersona } from "@buddy/backend/learning/shared/teaching-vocabulary"
export type {
  PersonaCatalogEntry as BuddyPersonaCatalogEntry,
  PersonaDefinition as BuddyPersonaProfile,
  PersonaOverride as BuddyPersonaOverride,
} from "../core/runtime/types-model"
export type {
  Persona as BuddyPersona,
  PersonaSurface as BuddySurface,
} from "@buddy/backend/learning/shared/teaching-vocabulary"
