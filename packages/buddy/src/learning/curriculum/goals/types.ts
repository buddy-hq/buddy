...
function isLikelyTaskLikeStatement(value: string): boolean {
  ...
  return /\b(build|create|design|implement|debug|solve|compare|analyze|evaluate|justify|explain)\b/.test(
    lower,
  )
}
...
function isLikelyTooBroadTask(value: string): boolean {
  ...
  return /\b(all aspects|everything|end-to-end|entire|full stack|completely|from scratch)\b/i.test(
    normalized,
  )
}
