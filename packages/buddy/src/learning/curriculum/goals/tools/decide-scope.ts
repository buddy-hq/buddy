...
const goalDecideScopeTool = createBuddyTool("goal_decide_scope", {
  ...
  async execute(params, ctx: BuddyToolContext) {
    ...
    const explicitlyRequestedSingleGoal = /\b(one|single)\s+goal\b/.test(lower) || /\ba\s+goal\b/.test(lower)

    const explicitScope = params.explicitScope
    const courseSignals = ["curriculum", "program", "overall", "whole course", "learning plan", "roadmap", "outcomes"]
    const topicSignals = [
      "module",
      "unit",
      "lesson",
