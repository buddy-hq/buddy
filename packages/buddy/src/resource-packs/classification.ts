...
const DIRECT_TEXT_EXTENSIONS = new Set([".txt", ".md", ".markdown", ".json", ".jsonc", ".yaml", ".yml", ".csv"])
...
export function classifyResourcePath(sourcePath: string, sourceSizeBytes?: number): ResourceClassification {
  ...
  if (DIRECT_TEXT_EXTENSIONS.has(extension)) {
    return {
      kind: sourceSizeBytes !== undefined && sourceSizeBytes > RESOURCE_PACK_LARGE_TEXT_THRESHOLD_BYTES
        ? "pack"
        : "direct",
      format: resourceFormatForExtension(extension),
      mime: "text/plain",
