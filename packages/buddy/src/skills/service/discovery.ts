...
import { SkillServiceError, type OpenCodeSkill } from "./contracts.js"
import { loadManagedSkillFile } from "./documents.js"
import { OPENCODE_SKILL_CACHE_ROOT, isWithinPath, managedSkillsRoot } from "./paths.js"
  ...
  const [openCodeSkills, managedSkills] = await Promise.all([
    options?.refresh
      ? Promise.all([
        ...
        ]).then(([cachedSkills, freshLocalSkills]) => mergeRefreshedOpenCodeSkills(cachedSkills, freshLocalSkills))
      : loadCachedOpenCodeSkills(directory),
