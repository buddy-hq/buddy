...
const ADVANCED_MATH_EXECUTABLE = process.platform === "win32" ? "buddy-advanced-math.exe" : "buddy-advanced-math"
...
const advancedMathRuntimeStateSchema = z.object({
  ...
  state: z.enum(["not_installed", "downloading", "installing", "ready", "repairing", "removing", "error"]),
...
function releaseRepository() {
  return process.env.BUDDY_REPO?.trim() || process.env.GITHUB_REPOSITORY?.trim() || DEFAULT_RELEASE_REPOSITORY
}
...
async function downloadBytes(url: string) {
  ...
  if (!response.ok) {
    throw new Error(`Failed to download advanced math runtime asset: ${response.status} ${response.statusText}`)
  }
...
async function downloadText(url: string) {
  const response = await fetch(url)
  if (!response.ok) {
    throw new Error(`Failed to download advanced math runtime checksum: ${response.status} ${response.statusText}`)
  }
...
function currentLocalAssetChecksumState() {
  ...
  try {
    return {
      ...
      checksum: parseChecksum(fs.readFileSync(localDevelopmentAssetPath(releaseChecksumFilename()), "utf8")),
    }
  } catch (error) {
    return {
      present: true as const,
...
function nextStatus(state: z.infer<typeof advancedMathRuntimeStateSchema>): AdvancedMathRuntimeStatus {
  ...
  const localAssetError =
    state.enabled && versionMatches && executableExists && localAssetState.present
      ...
      : undefined
  const ready = state.enabled && state.state === READY_STATE && versionMatches && executableExists && !localAssetError
...
async function installBundleFromRelease(reportProgress: (input: RuntimeProgressUpdate) => Promise<unknown>) {
  ...
  await reportProgress({
    ...
    progressPercent: 15,
    progressMessage: localSource ? "Reading local runtime bundle..." : "Downloading runtime bundle...",
  })
  ...
  await reportProgress({
    state: localSource ? "installing" : "downloading",
    progressPercent: 30,
    progressMessage: localSource ? "Loading runtime checksum..." : "Downloading runtime checksum...",
  })
  ...
  try {
    ...
    logRuntimeEvent("install-running-self-check", {
      executablePath: path.join(tempInstallRoot, ADVANCED_MATH_BUNDLE_DIR, ADVANCED_MATH_EXECUTABLE),
    })
    await ensureExecutablePermissions(path.join(tempInstallRoot, ADVANCED_MATH_BUNDLE_DIR, ADVANCED_MATH_EXECUTABLE))
    await runSelfCheck(path.join(tempInstallRoot, ADVANCED_MATH_BUNDLE_DIR, ADVANCED_MATH_EXECUTABLE))
    await reportProgress({
      state: "installing",
      progressPercent: 95,
...
function currentStatus() {
  ...
  if (runtimeOperation === undefined && IN_PROGRESS_STATES.has(status.state)) {
    return {
      ...
      lastError: status.lastError ?? "The previous advanced math runtime operation was interrupted.",
      progressPercent: undefined,
      progressMessage: undefined,
      ready: false,
    }
...
async function installRuntime() {
  return withRuntimeOperation(async () => {
    ...
    await updateRuntimeState({
      ...
      progressPercent: 5,
      progressMessage: repairing ? "Preparing runtime repair..." : "Preparing runtime installation...",
    })
...
export const AdvancedMathRuntimeService = {
  ...
  async runCalculator(code: string, abort?: AbortSignal) {
    ...
    if (!execution.result.ok) {
      throw new Error(execution.result.error?.trim() || execution.result.stderr.trim() || "Calculator execution failed")
    }
