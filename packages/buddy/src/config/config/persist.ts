...
import { parseConfigText, patchJsoncDocument } from "../document.js"
import { JsonError } from "../errors.js"
import { resetGlobalConfigCache } from "./cache.js"
import { resolveGlobalConfigFile, resolveProjectConfigContext, resolveProjectConfigFile } from "./paths.js"
...
export async function setProjectMcpConfig(directory: string, name: string, mcp: Mcp): Promise<void> {
  const { configDirectory } = await resolveProjectConfigContext(directory)
  const filepath = resolveProjectConfigFile(configDirectory)
  await ensureParentDirectory(filepath)
