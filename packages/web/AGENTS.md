...
- Workspace filters must use the exact package name `@buddy/web`; `web` is not a valid Turbo filter.
- Package-local typecheck is `bun run typecheck`; lint remains root-only through `bun run lint`.
