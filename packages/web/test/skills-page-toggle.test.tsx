...
describe("SkillsPage external vendor roots toggle", () => {
  ...
  test("loads initial toggle state and forces refresh after updating settings", async () => {
    ...
    await waitForAssertion(() => {
      expect(requests.some((request) => request.url === "/api/skills" && request.method === "GET")).toBe(true)
    ...
    await waitForAssertion(() => {
      ...
      expect(requests.some((request) => request.url === "/api/skills?refresh=1" && request.method === "GET")).toBe(
        true,
      )
