...
describe("advanced math runtime state", () => {
  test("uses the advanced math runtime lifecycle endpoints", async () => {
    ...
    globalThis.fetch = (async (input, init) => {
      ...
      if (matchesPath(String(input), "/api/local-runtimes/advanced-math/install") && init?.method === "POST") {
        return jsonResponse({
          enabled: true,
          state: "ready",
          ready: true,
      ...
      if (matchesPath(String(input), "/api/local-runtimes/advanced-math/install") && init?.method === "DELETE") {
        return jsonResponse({
          enabled: false,
          state: "not_installed",
          ready: false,
    ...
    expect(requests.every((request) => request.url.endsWith("/api/local-runtimes/advanced-math") || request.url.endsWith("/api/local-runtimes/advanced-math/install"))).toBe(true)
  })
