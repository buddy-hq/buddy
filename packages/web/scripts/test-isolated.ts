...
console.log(
  `[test:web] shard ${TEST_SHARD.index + 1}/${TEST_SHARD.count}: ${totalFileCount} files, ${results.length}/${plan.length} processes completed with concurrency ${TEST_CONCURRENCY} in ${formatDuration(performance.now() - startedAt)}`,
)
