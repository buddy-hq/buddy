...
function sourceWithFingerprint(source: ReaderSource, fingerprint: string | undefined): ReaderSource {
...
function isPdfPosition(anchor: ReaderRelocation["anchor"] | undefined): anchor is PdfPositionAnchor {
...
export const PdfReader = forwardRef<DocumentReaderHandle, PdfReaderProps>(
  function PdfReader(
    {
      source,
      className,
    ...
    const [documentState, setDocumentState] =
      useState<ReaderDocumentState>(initialDocumentState)
    ...
    const handleScaleChange = useCallback(
      (nextScale: number, presetValue: string | undefined) => {
      ...
      const session = new PdfViewerSession({
        ...
        callbacks: {
          onReady: (nextSnapshot, fingerprint) => {
            ...
            const stored = repository.loadDocument(
              persistenceSource,
              persistenceSuffix,
            )
            ...
            const restored = explicitInitialLocation ??
    ...
    const toggleBookmark = useCallback(() => {
      ...
      setDocumentState((current) => {
        const existing = current.bookmarks.find((bookmark) => isCurrentBookmark(bookmark, currentAnchor))
        if (existing) {
          return {
            ...current,
          ...
          (nextProgress) => {
            queueSearchProgress(
              nextProgress.totalPages
                ? nextProgress.completedPages / nextProgress.totalPages
                : 0,
            )
          },
          (pageResults) => {
    ...
    const cycleSearch = useCallback(
      (direction: 1 | -1) => {
        ...
        const nextIndex =
          (Math.max(0, currentIndex) + direction + results.length) % results.length
    ...
    return (
      ...
      >
        ...
        >
          ...
          {status === "ready" &&
          shouldShowPdfPageTurnControls(mode, layoutFallback !== null) ? (
            <>
              <Button
                type="button"
                variant="outline"
        ...
        <ReaderHelpDialog open={helpOpen} onOpenChange={setHelpOpen} shortcuts={PDF_READER_SHORTCUTS} />
      </section>
    )
  },
)
