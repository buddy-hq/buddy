  ...
  return (
    <div className="flex items-start justify-between gap-2">
      <div className="flex min-w-0 flex-wrap items-center gap-2">
        <span className={cn("text-sm font-medium text-foreground", running && "animate-pulse")}>{info.title}</span>
        {info.subtitle ? <span className="truncate text-sm text-muted-foreground">{info.subtitle}</span> : null}
        {info.detail ? <span className="truncate text-sm text-muted-foreground">{info.detail}</span> : null}
        {info.args?.map((arg) => (
