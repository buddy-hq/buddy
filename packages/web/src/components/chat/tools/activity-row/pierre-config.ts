...
[data-file] {
  ...
  --diffs-bg-selection-number: var(--diffs-bg-selection-number-override, rgb(from var(--surface-warning-base) r g b / 0.85));
...
:host([data-color-scheme='dark']) [data-file] {
  ...
  --diffs-bg-selection-number: var(--diffs-bg-selection-number-override, rgb(from var(--solaris-dark-6) r g b / 0.85));
}
...
function createPierreOptions(diffStyle: "unified" | "split") {
  return {
    theme: "OpenCode",
    themeType: "system",
...
export const PIERRE_CONTENT_OPTIONS = {
  ...createPierreOptions("unified"),
  overflow: "scroll",
} as const

export const PIERRE_DIFF_OPTIONS = createPierreOptions("unified")
