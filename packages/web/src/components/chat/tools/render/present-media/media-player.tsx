...
function BuddyMediaTransport(props: {
  ...
  const isCurrentPlayback = usePresentedMediaPlaybackStore(
    (state) => state.playingKey === props.playbackKey,
  )
  const [playing, setPlaying] = useState(false)
  const [currentTime, setCurrentTime] = useState(0)
  const [duration, setDuration] = useState(0)
  const [error, setError] = useState<string | undefined>(undefined)
  const [hasLoadedFrame, setHasLoadedFrame] = useState(!isVideo)
  ...
  const togglePlayback = useCallback(async () => {
    ...
    try {
      setError(undefined)
      if (media.paused) {
        requestPlayback(props.playbackKey)
        await media.play()
      } else {
        media.pause()
      }
    } catch (playbackError) {
      pausePlayback(props.playbackKey)
      setError(playbackError instanceof Error ? playbackError.message : String(playbackError))
    }
  }, [pausePlayback, props.playbackKey, requestPlayback])
      ...
      {isVideo ? (
        <div ref={shellRef} className={VIDEO_STAGE_CLASS_NAME}>
          ...
          <button
            ...
            aria-label={playing ? `Pause ${props.item.fileName}` : `Play ${props.item.fileName}`}
          />
          <video
            ref={(element) => {
            ...
            onError={() => setError("This media could not be played.")}
            className={cn(
...
export function PresentedMediaPlayer(props: {
  ...
  const isLoaded = usePresentedMediaPlaybackStore((state) =>
    state.loadedKeys.includes(props.playbackKey),
  )
  ...
  const rawUrl = props.item.rawUrl ? resolveAssetUrl(props.item.rawUrl) : undefined

  useEffect(() => {
    if (!canPreview || !rawUrl || !props.shouldLoad || isLoaded) return
    ensureLoaded(props.playbackKey)
  }, [canPreview, ensureLoaded, isLoaded, props.playbackKey, props.shouldLoad, rawUrl])

  if (!canPreview || !rawUrl) return props.fallback

  if (!isLoaded) {
    return props.shouldLoad ? (
      <MediaLoadingState item={props.item} compact={props.compact} />
    ) : (
      <MediaPlaceholder
        item={props.item}
        compact={props.compact}
        onActivate={() => ensureLoaded(props.playbackKey)}
      />
    )
  }

  return <BuddyMediaTransport item={props.item} playbackKey={props.playbackKey} src={rawUrl} />
}
