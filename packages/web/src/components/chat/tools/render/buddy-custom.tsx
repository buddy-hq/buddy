...
import { ACTIVITY_ROW_ERROR_CLASS_NAME, ACTIVITY_ROW_TEXT_CLASS_NAME } from "../activity-row/styles"
...
export function renderBuddyCustomTool({ state, info, icon }: ToolPartProps) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {hasError ? (
        <pre className={`${ACTIVITY_ROW_ERROR_CLASS_NAME} max-h-60 overflow-auto`}>{output}</pre>
      ) : !denied && hasContent ? (
        <pre className={`${ACTIVITY_ROW_TEXT_CLASS_NAME} max-h-60 overflow-auto`}>{output}</pre>
      ) : null}
      {!denied && !hasContent && valueText ? (
        <pre className={`${ACTIVITY_ROW_TEXT_CLASS_NAME} max-h-60 overflow-auto`}>{valueText}</pre>
      ) : null}
