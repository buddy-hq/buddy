...
export type MermaidViewportMode = "inline" | "bench"
...
export function resolveMermaidInitialZoomState(
  viewportMode: MermaidViewportMode | undefined,
...
function hasMeasuredViewportSize(viewportSize: MermaidViewportSize) {
...
export function useMermaidViewport({
  ...
  useEffect(() => {
    pendingSvgBoundsRef.current = false
  }, [svgBounds])
  ...
  useEffect(() => {
    ...
    pendingSvgBoundsRef.current = true
    setSvgBounds(measureSvgBounds(value.svg))

    if (isControlled) {
      autoFitRef.current = resolvedZoomState.isAutoZoom
      initializedRef.current = !resolvedZoomState.isAutoZoom
      setIsInitialized(!resolvedZoomState.isAutoZoom)
      return
    }
  ...
  useEffect(() => {
    if (
      ...
      pendingSvgBoundsRef.current ||
      fitFrameRef.current !== undefined ||
      !hasMeasuredViewportSize(viewportSize)
    ) {
      return
    }
