...
export function useTeachingWorkspace(props: UseTeachingWorkspaceProps) {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    const probe = probeTeachingWorkspace({ directory: decodedDirectory, sessionID })
      .then((workspace) => {
  ...
  async function onTeachingCheckpoint() {
    ...
    try {
      await checkpointTeachingWorkspace({ directory: decodedDirectory, sessionID })
      useTeachingRuntime.getState().setSaveError(sessionKey, undefined)
  ...
  async function onTeachingRestoreAccepted() {
    ...
    try {
      const workspace = await restoreTeachingWorkspace({ directory: decodedDirectory, sessionID })
      useTeachingRuntime.getState().setWorkspace(sessionKey, workspace)
      useTeachingRuntime.getState().setSaveError(sessionKey, undefined)
    } catch (restoreError) {
