...
export function useTeachingWorkspace(props: UseTeachingWorkspaceProps) {
  const { decodedDirectory, sessionID, sessionKey, isInteractiveMode, isBusy } =
    props;
  ...
  const workspaceProbeBySessionRef = useRef(
    new Map<
      string,
      Promise<Awaited<ReturnType<typeof loadTeachingWorkspace>> | undefined>
    >(),
  );
  ...
  useEffect(() => {
    if (!decodedDirectory || !sessionID || !sessionKey || teachingWorkspace)
      return;
    ...
    if (previousBusyRef.current && !isBusy) {
      void loadTeachingWorkspace({ directory: decodedDirectory, sessionID })
        ...
        .catch((workspaceError) => {
          useTeachingRuntime
            .getState()
            .setSaveError(sessionKey, stringifyError(workspaceError));
        });
    }
  ...
  async function flushTeachingWorkspace(input?: {
    ...
    const expectedRevision =
      input?.forceOverwrite && latest.conflict
        ? latest.conflict.revision
        : latest.revision;
    ...
    const task = (async () => {
      ...
      try {
        ...
        useTeachingRuntime
          .getState()
          .setSaveError(sessionKey, stringifyError(saveError));
        return false;
      }
    })();
  ...
  async function onTeachingSelectFile(relativePath: string) {
    ...
    try {
      ...
      useTeachingRuntime
        .getState()
        .setSaveError(sessionKey, stringifyError(fileError));
    }
  }
  ...
  async function onTeachingCheckpoint() {
    ...
    try {
      ...
      useTeachingRuntime
        .getState()
        .setSaveError(sessionKey, stringifyError(checkpointError));
    }
  }
  ...
  async function onTeachingRestoreAccepted() {
    ...
    try {
      ...
      useTeachingRuntime
        .getState()
        .setSaveError(sessionKey, stringifyError(restoreError));
    }
  }
