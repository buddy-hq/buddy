import { DEFAULT_THEME_ID, PRELOAD_STYLE_ID, STORAGE_KEYS, THEME_CACHE_VERSION, normalizeThemeID } from "./storage"
...
export function applyThemePreload(environment: ThemePreloadEnvironment) {
  ...
  const css = environment.storage.getItem(isDark ? STORAGE_KEYS.THEME_CSS_DARK : STORAGE_KEYS.THEME_CSS_LIGHT)
  if (!css) return
