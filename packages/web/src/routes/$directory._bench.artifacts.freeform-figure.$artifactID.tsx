...
function FreeformFigureBenchPending() {
  return (
    <BenchStaticContextProvider
      ...
      content="Freeform figure is visible on Bench and loading."
...
function FreeformFigureBenchError() {
  return (
    <BenchStaticContextProvider
      ...
      content="Freeform figure is visible on Bench, but it could not be loaded."
      hints={["Check that the freeform figure artifact exists."]}
    >
...
function FreeformFigureBenchRoute() {
  ...
  try {
    const directory = decodeDirectory(params.directory)
    return <FreeformFigureBenchView directory={directory} figure={figure} />
  } catch {
    return <DirectoryInvalidNotebook />
  }
}

function FreeformFigureBenchView(props: {
  directory: string
  ...
  const contextProvider = useMemo(
    () => ({
      read: () => ({
        ...
        content: [
          `Freeform figure: ${props.figure.title}`,
        ...
        refs: [
          artifactRef({
            artifactID: props.figure.artifactID,
            note: "Freeform figure artifact on Bench.",
          }),
          ...urlRef({
