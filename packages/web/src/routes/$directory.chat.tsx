...
function DirectoryChatPage() {
  ...
  useEffect(() => {
    if (decodedDirectory === "/") {
      ...
      if (fallback) {
        navigate({ to: "/$directory/chat", params: { directory: encodeDirectory(fallback) }, replace: true })
      } else {
    ...
    void ensureDirectorySession(decodedDirectory)
      .then((result) => {
        ...
        navigate({ to: "/$directory/chat", params: { directory: encodeDirectory(result.directory) }, replace: true })
      })
      .catch((error) => {
  ...
  function onSwitchDirectory(nextDirectory: string) {
    ...
    navigate({ to: "/$directory/chat", params: { directory: encodeDirectory(nextDirectory) } })
  }
  ...
  async function onRenameSession(targetDirectory: string, targetSessionID: string, title: string) {
    ...
    if (!trimmed) return
    try {
      const updated = await updateSession({ directory: targetDirectory, sessionID: targetSessionID, title: trimmed })
  ...
  async function onCloseDirectory(targetDirectory: string) {
    ...
    if (nextDirectory) {
      navigate({ to: "/$directory/chat", params: { directory: encodeDirectory(nextDirectory) }, replace: true })
      return
    }
  ...
  async function onSend() {
    ...
    if (slashCommand) {
      if (isResourceLocalSlashCommandName(slashCommand.command.name)) {
        ...
        try {
          const handled = await handleResourceCommand(resourceCommand, { rawAttachments })
          if (handled) return
          restorePromptSnapshot(draftSnapshot)
        } catch {
          restorePromptSnapshot(draftSnapshot)
  ...
  function onSettingsModalOpenChange(nextOpen: boolean) {
    ...
    if (!nextOpen) chatConfig.onSettingsModalClose()
  }
    ...
    <div className="h-full w-full overflow-hidden bg-card">
      <div className="h-full w-full flex min-w-0">
        ...
        <div
          ...
          style={{ width: `${cs.leftSidebarOpen ? cs.leftSidebarDisplayWidth : 0}px` }}
        >
          <div
        ...
        <div
          ...
          style={{ width: `${cs.rightSidebarOpen ? cs.rightSidebarDisplayWidth : 0}px` }}
        >
          <div
            className={`h-full transition-opacity duration-200 ease-out ${
          ...
          >
            <ChatRightSidebar
                    ...
                    <section className="flex min-h-0 flex-1 flex-col justify-center gap-4 px-6 py-8">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            teachingWs.onTeachingPreferredLanguageChange(event.target.value as TeachingLanguage)
                          }
              ...
              figurePanel={
                cs.selectedPersonaSupportsFigure ? <MathFigurePanel className="h-full min-h-0 flex-1" /> : undefined
              }
