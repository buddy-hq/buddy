  ...
  "mb-2 flex shrink-0 items-center justify-center [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        ...
        icon: "bg-surface-weak text-text-base flex size-8 shrink-0 items-center justify-center rounded-lg [&_svg:not([class*='size-'])]:size-4",
      },
    },
    defaultVariants: {
      variant: "default",
...
function EmptyDescription({ className, ...props }: React.ComponentProps<"p">) {
  return (
    <div
      ...
      className={cn(
        "text-sm/relaxed text-text-weak [&>a:hover]:text-text-interactive-base [&>a]:underline [&>a]:underline-offset-4",
        className,
      )}
      {...props}
    />
