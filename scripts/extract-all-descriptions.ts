...
async function processFile(filePath: string) {
  ...
  if (!extracted) {
    console.log(`SKIP (no description): ${filePath}`)
    return
  }
  ...
  if (content.includes(importPath)) {
    console.log(`SKIP (already imported): ${filePath}`)
    return
  }

  // Write markdown file
  await writeFile(mdFilePath, extracted.description + "\n")
  console.log(`CREATE: ${mdFilePath}`)

  // Replace description FIRST (while indices are still valid)
  const replacement = `description: ${constantName},`
  let newContent =
    content.slice(0, extracted.startIndex) + replacement + content.slice(extracted.endIndex)

  // Then remove dedent import if it was used
  if (extracted.type === "dedent") {
