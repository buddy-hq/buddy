...
- Buddy activates progressive frames without a vendor patch by installing an adapter startup bridge on the shared App-runtime LLM and Session services. AI SDK `onInputDelta` is the primary low-latency producer; normalized `tool-input-delta` is a deduplicated fallback for native runtimes and callback failures. See [Whiteboard Streaming](streaming.md).
...
- The adapter startup patch is implemented in `packages/opencode-adapter/src/tool-input-delta-live.ts`; it resolves both live services from the memoized App runtime and does not patch vendored files.
- The adapter bridge tests pin the upstream normalized event and processor discard branch, exercise real AI SDK callback ordering, verify callback-before-part queueing and deduplication, verify startup installation, verify whiteboard-only fallback forwarding, and verify cleanup bounds.
...
- Buddy avoids streaming jitter by parsing only complete inner-array objects and skipping render work unless the complete drawable set changed. The first and newest complete drawable apply immediately; incomplete objects remain buffered until they parse.
