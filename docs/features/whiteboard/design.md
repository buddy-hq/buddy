...
- Add a Buddy-owned adapter startup patch that forwards AI SDK `onInputDelta` callbacks through the existing `message.part.delta` path, with normalized `tool-input-delta` events retained as a deduplicated fallback for native runtimes and callback failures. See [Whiteboard Streaming](streaming.md).
...
- Apply every complete streamed drawable and control object immediately, including the first and newest complete item. Incomplete objects remain buffered because they cannot pass the inner-array parse.
...
- Live model-time progressive frames use a Buddy-owned adapter startup patch: before server startup it patches the shared App-runtime LLM and Session services, forwards AI SDK `onInputDelta` callbacks immediately, and retains normalized `tool-input-delta` as a deduplicated fallback. See [Whiteboard Streaming](streaming.md).
