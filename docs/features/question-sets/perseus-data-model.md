...
Status: Architectural reference for Khan Academy Perseus item/widget data models, the answerful vs answerless lifecycle, and future non-MCQ interaction registries (`graded-group`, `graded-group-set`). Shipped MCQ product direction: [direction.md](./direction.md).

This note explains the Khan Academy Perseus mental model in plain language and maps it to Buddy question-set objects. Full Perseus article/widget rendering parity is out of scope.
