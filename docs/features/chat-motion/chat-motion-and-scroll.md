# Chat Motion and Scroll UX Specification

Status: **desired UX / design guidance**. This is not a verified description of current
transcript runtime behavior. The 30% turn-transition push is specified here only; it is
not implemented as a `0.3 * viewport` write in `packages/web`. For shipped scroll
ownership, see [chat-transcript/scroll-and-virtualization.md](../chat-transcript/scroll-and-virtualization.md).
