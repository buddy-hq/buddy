...
This document describes the post-refactor Bench architecture. `bench-refactor.md` remains the authoritative refactor plan and acceptance source; older design and implementation logs in this directory are historical unless they are explicitly updated to match this document.
...
- The directory-scoped workspace store owns committed docked visibility, active drawer, pending workspace intent, and transient layout state.
- Per-directory workspace persistence owns only durable visibility and last drawer.
...
The route may own one active Bench target. The docked workspace may be expanded or collapsed. Explorer or Library may be a drawer over that target. Selecting an item presents the target, commits expanded visibility, and closes the drawer.
...
Workspace persistence stores only per-directory docked visibility and last drawer. Legacy right-sidebar and old Bench persistence keys are discarded by versioned migration while unrelated UI preferences are preserved.
...
Reading-resource linkage, whiteboard domain state, model context history, and DevTools-owned diagnostics remain domain-owned and are not treated as sidebar presentation state.
