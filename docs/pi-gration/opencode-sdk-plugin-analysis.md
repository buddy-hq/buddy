...
> Buddy vendored version: 1.15.4 (3 patch versions behind, hooks identical)
> Assessed by: cross-referencing the hypothesis against 3 independent agents; this is the corrected, consensus-informed version.
...
- **Zero frontend changes** — the v1 message format is preserved, avoiding PI's 511-line `pi-transcript-view.ts`
- **Single runtime** — no PI bridge, no dual agent loop, no `opencode-tool-bridge.ts`
- **Effort: ~2,000-3,000 lines** — ~4x less than `pi-gration` (11,000) but ~2x more than my original 500-800 estimate

The real win is not "all patches are gone" but "the PI bridge is unnecessary." Dropping 8,000 lines of pi-backend + bridge code while keeping the same adapter patches as `main` is already a win. Incremental wins (removing patches one at a time as OpenCode adds hooks) can happen over time.
...
- Effort: ~2,000-3,000 lines (not 500-800). Still ~4x less than `pi-gration`.
...
| `packages/buddy/src/pi-backend/runtime.ts` | 1,571 | PI runtime (would be deleted entirely) |
...
|---|---|---|
| **`permission.ask`** | Defined in `packages/plugin/src/index.ts:260`. Zero calls to `plugin.trigger("permission.ask", ...)` anywhere in the repo. | **Does not block Buddy.** Permissions work through the SDK HTTP API (`GET /permission`, `POST /permission/{id}/reply`) + bus events (`permission.asked`, `permission.replied`). This is the same mechanism Buddy uses today on `main`. The hook being dead doesn't matter — the HTTP API is the actual permission surface. |
...
| **PI Transcript** (`PiTranscriptEntry`) | 8 message roles | Custom PI event bridge → mapped to v1 format | Requires `pi-transcript-view.ts` (511 lines) to translate PI format |

**Key insight:** The plugin approach keeps the v1 message format. Zero frontend changes. The PI path requires 511 lines of transcript translation. This is the single biggest cost difference between the two approaches.
...
**Impact:** This is the one area where the plugin approach requires retaining a vendored OpenCode patch. This is not a regression from `main` — `main` already uses this patch. But it means the promise of "no vendored internals hacking" is not fully achievable today.
...
| Remove PI backend entirely | `pi-backend/` (37 files) | -5,300 |
| No frontend changes | — | 0 |
| **Net** | | **~1,500-2,500 added, ~5,900 removed** |

**Pros:**
- System prompt guard (183 lines) becomes a published plugin hook — a clean win over `main`
- Partial win on other patches: 2 forwarding patches remain but could be consolidated into one
- No dual runtime (no PI bridge, no opencode-tool-bridge, no pi-backend/)
- Zero frontend changes (v1 message format preserved)
- Plugin gets full SDK client for API access
- Dramatically less code than `pi-gration` (~2,500 vs 11,000 lines — ~4x less)
...
- The patch reduction is real but modest (1 of 4), not the "3 of 4" I originally claimed

### Path 2: Continue PI + Bridge (pi-gration, current)

**Status:** 5,300 lines of new backend code + ~600 lines of frontend changes. LSP broken, write tool stubbed, file operations reimplemented, project info moved to JSON file. Dual tool system (Effect + PI). Three message format conversions. OpenCode tools still run through `opencode-tool-bridge.ts`.

**Pros:** Full control over agent loop. Works today (with bridge).

**Cons:** Maintenance burden of 8,000-line bridge. Every PI update is a breaking change risk. OpenCode tools still depend on vendored code through bridge. Massive code surface to maintain. PI lacks built-in tools (bash, edit, LSP). The bridge IS the new proxy layer — just bigger.

### Path 3: Continue vendored OpenCode + proxy (main, current)
...
**Cons:** 4 vendored internals patches to maintain. Proxy layer is brittle. Every new feature requires understanding both Buddy's code and OpenCode's internals. The `pi-gration` experiment was motivated by real pain here.

### Path 4: SDK alone (no internals, no plugin)
...
### Current `pi-gration` Bridge (removed entirely)

| Component | Lines | 
|---|---|
| `pi-backend/` (37 files) | 5,300 |
| `opencode-tool-bridge.ts` | 319 |
| `opencode-messages.ts` | 314 |
| `opencode-environment.ts` | 65 |
| Various PI-specific learning files | ~300 |
| **Total removed** | **~6,000** |
...
| Remove PI backend | -5,300 | Delete 37 files |
| | Test updates | ~200-300 | Updated tests for removed/replaced code |
| **Net new** | **~2,000-3,000** | ~4x less than pi-gration, but ~2x my original estimate |

### The PI Path (pi-gration, current)

| Work item | LOC | Notes |
|---|---|---|
| pi-backend/ (new) | 5,300 | 37 files |
| Frontend changes (new + modified) | ~600 | pi-transcript-view.ts (511) + various |
| Modified existing backend | ~2,000 | 50+ files |
| Tests | ~2,000 | 15+ files |
| Deleted files | -1,200 | 27 files |
| **Net new** | **~11,000** | **~5x more than plugin approach** |
...
2. **Keep one adapter patch** (`session-prompt-tool-forwarding.ts`) for subagent forwarding. Watch for upstream OpenCode to add a session spawn hook.
...
6. **Drop the `pi-gration` branch entirely.**
...
## Appendix: pi-gration Branch Inventory

| Category | Lines | Files | Examples |
|---|---|---|---|
| New PI backend (`pi-backend/`) | ~5,300 | 37 | `runtime.ts` (1,571), `actions.ts`, `mapper.ts`, `opencode-tool-bridge.ts`, `session-event-bridge.ts`, `ui-requests.ts` |
| New config/runtime files | ~250 | 4 | `config-access.ts`, `wildcard.ts`, `skills/paths.ts` |
| New learning/runtime utils | ~160 | 3 | `json-schema-typebox.ts`, `normalize-tool-call-args.ts`, `tool-result-metadata.ts` |
| New project info | ~320 | 2 | `project-info.ts` (282), `project-contract.ts` |
| New frontend transcript view | ~510 | 1 | `pi-transcript-view.ts` |
| Modified existing backend | ~2,000 | 50+ | Routes, session orchestration, learning features |
| Modified existing frontend | ~600 | 15+ | Chat store, types, message utils, hidden steps |
| Deleted files | ~1,200 | 27 | `proxy/`, `opencode-runtime/`, `http/session.ts` |
| Tests (new + modified) | ~2,000 | 15+ | PI backend tests, skill tests, chat store parity |
| **Total** | **~11,000** | **165** | |

### Comparison: pi-gration vs Plugin Approach

| Metric | pi-gration | Plugin Approach |
|---|---|---|
| New backend code | 5,300 lines (37 files) | ~2,000-3,000 lines (~15 files) |
| New frontend code | 600 lines | 0 lines |
| Adapter patches retained | 0 (but 319-line bridge + 8,000-line pi-backend replaces them) | 2-3 patches (122 lines, same as main — could consolidate to 2) |
| Agent runtimes | 2 (PI + vendored OpenCode via bridge) | 1 (OpenCode) |
| Message formats | 3 (PI, Buddy, OpenCode) | 1 (OpenCode v1) |
| Tool registration | Dual system (Effect + PI) | Single (plugin `tool` + Zod) |
| LSP | Broken (stubbed) | Works (OpenCode built-in) |
| Write tool | 15-line fs.writeFile stub | Full OpenCode write tool |
| File operations | Reimplemented | OpenCode built-in |
| Key dependency | PI libraries + vendored OpenCode bridge | OpenCode plugin system + 2-3 adapter patches |
| Patch reduction vs main | N/A (removes all patches, adds bridge) | 1 of 4 patches removed, 2-3 remain (real but modest improvement) |

---

## Appendix B: Architecture Comparison — What Moves Where
...
- **Built-in tools stay** (no PI bridge needed).
- **Zero frontend changes** (v1 message format preserved).
...
## Appendix C: Upstream Sync Pain Assessment
